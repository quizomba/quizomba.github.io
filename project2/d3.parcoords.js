d3.parcoords = function(config) {
  var __ = {
    ...
    width: 6000,
    height: 5000,
    margin: { top: 24, right:50, bottom: 24, left: 50 },
    nullValueSeparator: "top",  // set to "top" or "bottom"
    nullValueSeparatorPadding: { top: 8, right: 50, bottom: 8, left: 50 },
    color: "#069",
    composite: "source-over",
    alpha: 0.7,
    bundlingStrength: 50.5,
    bundleDimension: null,
    smoothness: 0,
    showControlPoints: false,
    hideAxis : [],
    flipAxes: [],
    animationTime: 100, // How long it takes to flip the axis when you double click
    rotateLabels: true,
...
pc.autoscale = function() {
  ...
  pc.selection.selectAll("canvas")
      ...
      .style("width", (w()+25) + "px")
      .style("height", (h()+25) + "px")
      .attr("width", (w()+25) * devicePixelRatio)
      .attr("height", (h()+25) * devicePixelRatio);

  // default styles, needs to be set when canvas width changes
  ctx.foreground.strokeStyle = __.color;
  ctx.foreground.lineWidth = 3;
  ctx.foreground.globalCompositeOperation = __.composite;
  ctx.foreground.globalAlpha = __.alpha;
  ctx.foreground.scale(devicePixelRatio, devicePixelRatio);
  ctx.brushed.strokeStyle = __.brushedColor;
  ctx.brushed.lineWidth = 3;
  ctx.brushed.opacity = 10;
  ctx.brushed.globalCompositeOperation = __.composite;
  ctx.brushed.globalAlpha = __.alpha;
  ctx.brushed.scale(devicePixelRatio, devicePixelRatio);
  ctx.highlight.lineWidth = 10;
...
pc.applyDimensionDefaults = function(dims) {
  ...
  dims.forEach(function(k) {
    ...
    newDims[k].ticks= newDims[k].ticks != null ? newDims[k].ticks : 0;
    newDims[k].innerTickSize= newDims[k].innerTickSize != null ? newDims[k].innerTickSize : 0;
    newDims[k].outerTickSize= newDims[k].outerTickSize != null ? newDims[k].outerTickSize : 0;
    newDims[k].tickPadding= newDims[k].tickPadding != null ? newDims[k].tickPadding : 0;
